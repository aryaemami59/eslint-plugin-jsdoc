...
type ESTreeOrTypeScriptNode = ESTreeNode | TSESTree.Node;
...
const isVoidPromise = (
  ...
  return (
    (node as TSESTree.TSTypeReference)?.typeArguments?.params?.[0]?.type ===
      "TSVoidKeyword" ||
    /* c8 ignore next 4 */
    // @ts-expect-error
    (node as TSESTree.TSTypeReference)?.typeParameters?.params?.[0]?.type ===
      "TSVoidKeyword"
  );
};
...
const hasReturnValue = (
  ...
  switch (node.type) {
    ...
    case "TSMethodSignature": {
      const type = (node as TSESTree.TSFunctionType)?.returnType?.typeAnnotation
        ?.type;
    ...
    case "ArrowFunctionExpression": {
      return (
        ("expression" in node &&
          node.expression &&
          (!isNewPromiseExpression(node.body) || !isVoidPromise(node.body))) ||
    ...
    case "IfStatement": {
      return (
        hasReturnValue(node.consequent, throwOnNullReturn, promFilter) ||
  ...
  switch (node.type) {
    ...
    case "ArrowFunctionExpression": {
      return (
        ...
        (node.body as TSESTree.BlockStatement).body.some((nde) => {
    ...
    case "DoWhileStatement":
      if (
         ...
         */
        (node as TSESTree.Literal).value === true
      ) {
          ...
          (!node.finalizer ||
            (() => {
              try {
                ...
                if ((error as Error).message === "Null return") {
                  return false;
                }
                /* c8 ignore next 2 */
                throw error;
    ...
    case "SwitchStatement": {
      return (node as TSESTree.SwitchStatement).cases.every((someCase) => {
...
type PromiseFilter = (node: ESTreeOrTypeScriptNode) => boolean;
  ...
  switch (node.type) {
    ...
    case "ArrowFunctionExpression": {
      ...
      if (
        ...
        node.params[0].name === resolverName
      ) {
        return false;
      }
    ...
    case "TryStatement": {
      return (
        ...
        hasNonEmptyResolverCall(node.handler?.body, resolverName) ||
...
const hasValueOrExecutorHasNonEmptyResolveValue = (
  ...
  const hasReturnMethod = allBranches
    ? (nde: ESTreeOrTypeScriptNode, promiseFilter: PromiseFilter): boolean => {
        let hasReturn: boolean | undefined;
        try {
  ...
  return hasReturnMethod(node, (prom) => {
    ...
    const { params, body } = (prom as TSESTree.NewExpression).arguments[0] as
