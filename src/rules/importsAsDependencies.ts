...
const moduleCheck = new Map<string, boolean>();
...
export default iterateJsdoc(
  ({
    ...
    jsdoc: { tags: { type: string }[] };
    settings: { mode: string };
    utils: {
      reportJSDoc: (message: string, tag: any) => void;
    };
  }) => {
    if (deps === undefined) {
      setDeps();
    ...
    for (const tag of jsdoc.tags) {
      ...
      try {
        typeAst =
          mode === "permissive" ? tryParse(tag.type) : parse(tag.type, mode);
      } catch {
        continue;
      }
          ...
          if (isBuiltinModule(mod)) {
            ...
            try {
              pkg = JSON.parse(
                // @ts-expect-error It's ok
                readFileSync(
                  join(process.cwd(), "node_modules", mod, "./package.json"),
                  "utf-8",
                ),
              );
            } catch {
