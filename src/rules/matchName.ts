...
export default iterateJsdoc<ContextOptions>(
  ({ context, jsdoc, report, info: { lastIndex }, utils }) => {
    ...
    const disallowNameRegex =
      disallowName && utils.getRegexFromString(disallowName);

    let applicableTags = jsdoc.tags as JsdocTag[];
    if (!tags.includes("*")) {
      applicableTags = utils.getPresentTags(tags) as JsdocTag[];
    }
    ...
    for (const tag of applicableTags) {
      ...
      utils.reportJSDoc(
        message,
        hasRegex ? tag : null,
        disallowNameRegex && replacement !== undefined ? fixer : null,
        false,
        {
          allowName,
          disallowName,
