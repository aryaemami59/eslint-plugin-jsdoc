...
const defaultEmptyTags: Set<string> = new Set([
  "abstract",
  "async",
  "generator",
  "global",
...
const emptyIfNotClosure: Set<string> = new Set([
  "package",
  "private",
  "protected",
  "public",
...
const emptyIfClosure: Set<string> = new Set(["interface"]);

export default iterateJsdoc(
  ({ settings, jsdoc, utils }: { settings: any; jsdoc: any; utils: any }) => {
    const emptyTags = utils.filterTags(({ tag: tagName }: { tag: string }) => {
      return (
        defaultEmptyTags.has(tagName) ||
        (utils.hasOptionTag(tagName) &&
          jsdoc.tags.some(({ tag }: { tag: string }) => {
            return tag === tagName;
          })) ||
