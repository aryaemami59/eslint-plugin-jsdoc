 ...
 * @returns {string}
 */
const htmlReplacer = (desc: string): string => {
 ...
 * @returns {string}
 */
const markdownReplacer = (desc: string): string => {
  return desc.replaceAll(
    new RegExp(markdownRegex, "gu"),
    (_, backticks, encapsed) => {
...
export default iterateJsdoc(
  ({ context, jsdoc, utils }) => {
    ...
    if (escapeHTML) {
      if (descriptions.some((desc: string) => htmlRegex.test(desc))) {
        const line = utils.setDescriptionLines(htmlRegex, htmlReplacer);
        utils.reportJSDoc(
          "You have unescaped HTML characters < or &",
          {
            line,
          },
          () => {},
      ...
      for (const tag of jsdoc.tags) {
        if (
          /** @type {string[]} */ utils
            .getTagDescription(tag, true)
            .some((desc: string) => htmlRegex.test(desc))
        ) {
          const line =
            utils.setTagDescription(tag, htmlRegex, htmlReplacer) +
            tag.source[0].number;
          utils.reportJSDoc(
    ...
    if (descriptions.some((desc: string) => markdownRegex.test(desc))) {
      const line = utils.setDescriptionLines(markdownRegex, markdownReplacer);
      utils.reportJSDoc(
        "You have unescaped Markdown backtick sequences",
        {
          line,
        },
        () => {},
    ...
    for (const tag of jsdoc.tags) {
      if (
        /** @type {string[]} */ utils
          .getTagDescription(tag, true)
          .some((desc: string) => markdownRegex.test(desc))
      ) {
        const line =
          utils.setTagDescription(tag, markdownRegex, markdownReplacer) +
          tag.source[0].number;
        utils.reportJSDoc(
