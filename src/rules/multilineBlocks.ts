...
export default iterateJsdoc<ContextOptions>(
  ({ context, jsdoc, utils }) => {
    const {
    ...
    const isInvalidSingleLine = (tagName: string) => {
      return (
        noSingleLineBlocks &&
        (!tagName ||
    ...
    if (noMultilineBlocks) {
      ...
      if (
        noSingleLineBlocks &&
        (!jsdoc.tags.length ||
          !utils.filterTags(({ tag: tg }) => !isInvalidSingleLine(tg)).length)
      ) {
        utils.reportJSDoc(
          "Multiline jsdoc blocks are prohibited by your configuration but fixing would result in a single line block which you have prohibited with `noSingleLineBlocks`.",
        );
      ...
      if (jsdoc.tags.length > 1) {
        ...
        if (!allowMultipleTags) {
          utils.reportJSDoc(
            "Multiline jsdoc blocks are prohibited by your configuration but the block has a description with a tag.",
          );
