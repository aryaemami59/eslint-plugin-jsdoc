...
export default iterateJsdoc(
  ({ context, utils }) => {
    const { noOptionalParamNames } = (context.options[0] || {}) as {
    ...
    for (const tag of paramTags) {
      if (noOptionalParamNames && tag.optional) {
        utils.reportJSDoc(
          `Optional param names are not permitted on @${tag.tag}.`,
          tag,
          () => {
            utils.changeTag(tag, {
        ...
        utils.reportJSDoc(
          `Defaults are not permitted on @${tag.tag}.`,
          tag,
          () => {
            utils.changeTag(tag, {
              name: tag.name.replace(/([^=]*)(=.+)?/u, "[$1]"),
    ...
    for (const tag of defaultTags) {
      if (tag.description.trim()) {
        utils.reportJSDoc(
          `Default values are not permitted on @${tag.tag}.`,
          tag,
          () => {
            utils.changeTag(tag, {
              description: "",
